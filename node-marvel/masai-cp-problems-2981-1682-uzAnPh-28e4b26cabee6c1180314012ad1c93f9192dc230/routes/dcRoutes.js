...
router.post("/addnew", (req, res) => {
  try {
    ...
    const newHero = { id: db.dc.length + 1, name, alias, power_level, role };
    db.dc.push(newHero);
...
router.get("/", (req, res) => {
  try {
    const db = JSON.parse(fs.readFileSync("db.json"));
    const dcData = req.query.alias
      ? db.dc.find((hero) => hero.alias === req.query.alias)
      : db.dc;
    res.status(200).json(dcData);
  } catch (error) {
    res.status(500).send(error.message);
  }
});
...
router.get("/:id", (req, res) => {
  try {
    const db = JSON.parse(fs.readFileSync("db.json"));
    const hero = db.dc.find((hero) => hero.id === parseInt(req.params.id));
    if (hero) {
...
// Endpoint: /dc/update/:id
router.patch("/update/:id", validatorMiddleware, (req, res) => {
  try {
    const db = JSON.parse(fs.readFileSync("db.json"));
    const { name, alias, power_level, role } = req.body;
    const heroIndex = db.dc.findIndex(
